...
from .decode import Interval, decode_all, unknown_regions
from .rasterise import rasterise, time_edges

__all__ = ["plot_kippenhahn"]

_XLABELS = {
    "timestep": "Model number",
    "index": "Model index",
    "age": "Age [yr]",
    "collapse": r"$\log_{10}\left(\text{time to end of run / yr}\right)$",
}
...
def _compute_x(data: dict[str, Any], xaxis: str) -> tuple[np.ndarray, str, bool]:
    ...
    if xaxis == "timestep":
        x = np.asarray(data["timestep"], dtype=float)
        x = _strictly_increasing(x)
        return x, _XLABELS[xaxis], False
    if xaxis == "index":
        x = np.arange(len(data["timestep"]), dtype=float)
        return x, _XLABELS[xaxis], False
    if xaxis == "age":
...
def plot_kippenhahn(
    ...
    *,
    ...
    co_line = ax.fill_between(x, 0, CO_core, color='navajowhite', alpha=0.6, zorder=4.5, label='CO core', hatch='ooo')
    ...
    #         color='pink', rasterized=True, zorder=0.6,
    #     )
    #     legend_handles.append(Patch(color="pink", label="Unknown"))
    ...
    if semiconv:
        ...
        legend_handles.append(Patch(color=semi_color, label="Semiconvective Zone"))
    ...
    legend_handles.append(Patch(color=conv_color, label="Convective Zone"))

    he_line = ax.fill_between(
        x, CO_core, He_core, color="teal", linewidth=0, zorder=4, label="He core", alpha=0.6, hatch='xxx'
    )
    ...
    ax.fill_between(
        x, M, M[0], color="black", alpha=0.15, lw=0, zorder=5, hatch='//'
    )
    ...
    ax.set_title(title)
    ax.legend(
        handles=legend_handles, ncol=3, frameon=True
    )
