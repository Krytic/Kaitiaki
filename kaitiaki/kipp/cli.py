...
_XAXIS_CHOICES = ("timestep", "index", "age", "collapse")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--xaxis", choices=_XAXIS_CHOICES, default="timestep", help="Quantity to use for the x axis.")
...
def main(plotfile) -> int:
    ...
    if isinstance(plotfile, str):
        try:
            ...
            print(f"kipp: error reading {plotfile!r}: {exc}", file=sys.stderr)
            return 1
        except ValueError as exc:
            print(f"kipp: error parsing {plotfile!r}: {exc}", file=sys.stderr)
            return 1
    else:
        data = plotfile

    intervals_per_model, bad_rows = decode_all(
        data["conv"], data["M"], conv_env=data["conv_env"]
    )
    n_models = len(data["timestep"])
    bad_models = [int(data["timestep"][i]) for i in bad_rows[:_MAX_BAD_MODELS_SHOWN]]
    print(
